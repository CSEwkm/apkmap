...
def banner_apk():
    ...
    | (_| | |_) |   <  / ____ \| | | | (_| | | |_| \__ \  __/ |
     \__,_| .__/|_|\_\/_/    \_\_| |_|\__,_|_|\__, |___/\___|_|
          | |    Code By Nicholas ☭           __/ |
          |_|https://github.com/Nicholas-lang |___/
...
# 创建Android任务
@cli.command(help="Get the key information from APK or DEX.")
@click.option("-i", "--inputs", required=True, type=str, help="Enter the APK file or DEX file to be scanned or the corresponding APK download address.")
@click.option("-r", "--rules", required=False, type=str, default="", help="Enter a rule for temporary scanning of file contents.")
@click.option("-s", "--sniffer", is_flag=True, default=True, help="Enable the network sniffer function. It is on by default, close it to be faster.")
...
@click.option("-a", '--all',is_flag=True, default=True,help="Output the string content that conforms to the scan rules.It is on by default.")
...
def android(inputs: str, rules: str, sniffer: bool, no_resource:bool, all:bool, threads:int, output, package:str) -> None:
    ...
    try:
        ...
        bootstrapper.init()
        bootstrapper.begin("Android", inputs, rules, sniffer, threads, package)
