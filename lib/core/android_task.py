...
class AndroidTask(object):
    ...
    def start(self):
        ...
        if os.system("java -version") !=0 :
            raise Exception("[!]Please install the Java environment for running apktool! Recommended Java 8 (JRE 1.8).")
        ...
        if os.path.isdir(input_file_path):
           ...
           if self.__decode_file__(input_file_path) == "error":
               raise Exception("[!]Retrieval of this file type is not supported! Please select APK file or DEX file.")
    ...
    def __decode_file__(self,file_path):
        ...
        if suffix_name == "apk":
            ...
            output_path = os.path.join(base_out_path,name+'_'+str(create_time))
            ...
            try:
                ...
                print("[*]Create output folder '{}' for DEX.".format(output_path))
            except FileExistsError:
                print("[!]folder '{}' already exists, it will be overwritten!".format(output_path))
    ...
    def __decode_dex__(self,file_path,backsmali_path,output_path):
        cmd_str = ("java -jar %s d %s -o %s") % (backsmali_path,str(file_path),str(output_path))
        if os.system(cmd_str) == 0:
    ...
    def __get_scanner_file__(self,scanner_dir,scanner_file_suffixs=["smali"]):
        ...
        for dir_or_file in dir_or_files:
            ...
            if os.path.isdir(dir_file_path):
                ...
                # 判断文件后缀名是否在scanner_file_suffixs中
